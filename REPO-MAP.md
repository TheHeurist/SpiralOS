...
## Strategic rationale
- The root remains compact so covenant and orientation are never downstream dependencies.
- `docs/` carries explainability/public legibility without diluting kernel authority.
- `runtime/` and `agents/` allow implementation growth while preserving non-overwrite constraints.
- `provenance/` keeps chain-of-trust explicit across publication and reuse.

## Claim-lane preservation
In maps, specs, and interfaces, maintain separate lanes:
- `canonical_claims` (stabilized)
- `frontier_claims` (exploratory)
- `fhs_claims` (floating-hypothesis-space)
